...
- 【2024-11】[OpenAI’s Approach to External Red Teaming for AI Models and
Systems](https://arxiv.org/pdf/2503.16431v1)（arXiv:2503.16431v1）
  ...
  <details>
    ...
    - **主要内容**：OpenAI 在 AI 模型和系统的外部红队测试（Red Teaming）方面的实践，强调其在 识别 AI 风险、优化安全措施、提升评估体系以及增强公信力 方面的作用。红队测试通过模拟对抗性攻击和潜在滥用场景，帮助发现模型在不同应用环境下可能带来的新型风险，并用于测试现有安全机制的有效性。文章详细介绍了如何设计红队测试，包括 选择测试人员（确保背景、技能、地域和语言的多样性）、决定测试范围（不同阶段的模型版本开放程度）、提供测试指南（确保可复现性和可操作性），以及如何将人工红队测试结果转化为自动化评估，以提升覆盖率和效率。OpenAI 通过红队测试改进了 GPT-4、DALL·E 3 等模型的安全性，例如发现并修复了绕过内容过滤系统的攻击方法，以及防止 GPT-4o 在语音模式下意外模仿用户声音等问题。尽管红队测试在 AI 风险评估中具有重要价值，文章也指出了其局限性，如高昂的成本、可能对测试者造成心理负担、测试结果的时效性问题以及信息泄露的潜在风险。因此，OpenAI 强调，红队测试应与 自动化评估、安全基准、第三方审计等措施 结合使用，以构建更加全面的 AI 评估体系。随着 AI 技术的快速发展，红队测试方法也需要不断演进，以确保 AI 系统的 安全性、可靠性和可控性，并为 政策制定者、研究人员和 AI 开发者 提供更有效的风险评估工具。
